...
test("app UI stays locked to Roboto while Konva canvas fonts remain unchanged", () => {
  const uiFiles = [
    "src/index.css",
  ...
  assert.match(uiSource, /--font-sans:\s*'Roboto'/);
  assert.match(uiSource, /--font-display:\s*'Roboto'/);
  assert.doesNotMatch(
    uiSource,
    /["'](?:Inter|Syne|DM Sans|Space Mono|SF Pro Text)["']|system-ui|-apple-system|fontFamily:\s*["']monospace/,
  );
